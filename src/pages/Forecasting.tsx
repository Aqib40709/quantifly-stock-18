...
export default function Forecasting() {
  ...
  const loadForecasts = async () => {
    ...
    if (forecastData && forecastData.length > 0) {
      ...
      const processed = Object.values(grouped).map((item: any) => ({
        ...
        reorder_suggestion: Math.max(item.reorder_level, Math.round(item.predicted_demand / item.count * 1.2)),
      }));

      setForecasts(processed as ForecastData[]);
      
      const avgConfidence = processed.reduce((sum: number, f: any) => sum + f.confidence_score, 0) / processed.length;
  ...
  const generateSimpleForecasts = async () => {
    ...
    const simpleForecasts = Object.entries(productSales).map(([name, data]: [string, any]) => {
      ...
      const predicted = Math.round(avgDaily * 30); // 30-day forecast
      return {
        product_name: name,
        predicted_demand: predicted,
        confidence_score: 0.7, // Moderate confidence for simple forecasts
                ...
                {forecasts.map((forecast, index) => (
                  <TableRow key={index}>
                    ...
                    <TableCell className="text-right">
                      <span className={
                        forecast.confidence_score >= 0.8 ? "text-green-500" :
